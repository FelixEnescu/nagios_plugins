...
MONIT_HOST="monitor.qsol.ro"

/usr/bin/printf "%b" "Notification Type: $NOTIFICATIONTYPE\n\nService: $SERVICEDESC_CLEAN\nHost: $HOSTNAME\nHost alias: $HOSTALIAS\nAddress: $HOSTADDRESS\nState: $SERVICESTATE\nDate/Time: $LONGDATETIME\n\nAdditional Info:$SERVICEOUTPUT\n\nService Perfdata: http://$MONIT_HOST/pnp4nagios/index.php/graph?host=$HOSTNAME&srv=$SERVICEDESC_CLEAN\nHost Information: http://$MONIT_HOST/icinga/cgi-bin/extinfo.cgi?type=1&host=$HOSTNAME\nHost Service Detail: http://$MONIT_HOST/icinga/cgi-bin/status.cgi?host=$HOSTNAME&nostatusheader\n" | /bin/mail -s "** $NOTIFICATIONTYPE: $HOSTALIAS/$SERVICEDESC is $SERVICESTATE **" $CONTACTEMAIL
